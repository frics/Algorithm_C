...
void init_element();      // 값입력받음& 초기화
void optsearchtree();      // 최적이진검색트리구축을위한배열R계산
void make_tree();      // 최적이진검색트리구축시작함수
void search();            // 이진트리검색함수
void print_r();            // 배열R 출력
void print_minavg();                                                //  평균 검색 시간 출력
...
void init_element()
{
    ...
    for (diagonal = 1; diagonal <= n - 1; diagonal++)
        for (i = 1; i <= n - diagonal; i++) {
            j = i + diagonal;
            a[i][j] = minimum(a, i, j, &k) + sigma(i, j);                                //  일반식 구현
